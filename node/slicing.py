...
def validate_params(
    ...
    if not xor(first is not None, last is not None):
        raise exceptions.IncorrectInputException(
            message="One of the following parameters must be specified (not both): `first`, `last`."
        )
    if first and first <= 0:
        raise exceptions.IncorrectInputException(
            message="`first` parameter must be more than 0"
        )
    elif last and last <= 0:
        raise exceptions.IncorrectInputException(
            message="`last` parameter must be more than 0"
        )
